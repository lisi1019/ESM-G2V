...
class Model(object):
    ...
    def split_and_classify(self):
        ...
        predicted, predicted_prob = self.model_fit(X_train,  X_test, y_train,  y_test)
...
class NNClf(Model):
    ...
    def set_clf(self, n):
        model = tf.keras.models.Sequential([
            tf.keras.layers.Dense(256, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.01)),
            tf.keras.layers.Dropout(0.1),
            tf.keras.layers.Dense(128, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.01)),
            tf.keras.layers.Dropout(0.1),
            tf.keras.layers.Dense(64, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.01)),
            tf.keras.layers.Dropout(0.1),
            tf.keras.layers.Dense(n, activation=tf.nn.softmax)
        ])
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.00001)
      
        model.compile(loss=SparseCategoricalFocalLoss(gamma=2),optimizer=optimizer, metrics=['accuracy'])
        self.clf = model
    def model_fit(self, X_train, X_test, y_train, y_test):
        ...
        y_test_tag = np.array([inverse_dic[y] for y in y_test])
        ...
        history = clf.fit(x=X_train, y=y_train_tag, batch_size=256,
                          ...
                          validation_data=(X_test, y_test_tag),
                          callbacks=[early_stopping])
        self.history = history
        clf.save('/root/fsas/ESM-G2V/model/ESM_gene2vec.h5')
    ...
    def evaluate(self, X_test, y_test):
        ...
        loss, accuracy = self.clf.evaluate(X_test, y_test_tag)
...
class NNClfFolds(FoldsModel):
    ...
    def set_clf(self, n):
        model = tf.keras.models.Sequential([
            ...
            tf.keras.layers.Dropout(0.1),  # Increased dropout rate
            tf.keras.layers.Dense(32, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.01)),
    ...
    def model_fit(self, X_train, X_test, y_train, y_test):
        ...
        # Custom callback to log test set loss
        ...
        history = clf.fit(x=X_train, y=y_train_tag, batch_size=256, epochs=50,
                          validation_data=(X_test, y_test_tag),
                          shuffle=True, verbose=0, callbacks=[CustomCallback()])
        self.history = history
    ...
    def evaluate(self, X_test, y_test):
        ...
        dic_y_mapping = {n: label for n, label in enumerate(np.unique(self.y))}
        inverse_dic = {v: k for k, v in dic_y_mapping.items()}
        y_test_tag = np.array([inverse_dic[y] for y in y_test])
